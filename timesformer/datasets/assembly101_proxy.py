...
def _resolve_distill_modalities(exo_modality):
    ...
    if not modalities:
        raise ValueError("assembly101_proxy requires at least one distillation modality.")
    return list(dict.fromkeys(modalities))


@DATASET_REGISTRY.register()
class Assembly101_proxy(torch.utils.data.Dataset):
    """
    Assembly-101 proxy dataset aligned with Egoexo_fitness_proxy/Egoexo4d_proxy.
    """
    ...
    def __init__(self, cfg, mode, num_retries=10):
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for Assembly-101".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing Assembly-101 proxy {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        ), "Failed to load Assembly-101 split {} from {}".format(
            split_name, path_to_file
        )
        logger.info(
            "Constructing Assembly-101 proxy dataloader (size: {}) from {}".format(
