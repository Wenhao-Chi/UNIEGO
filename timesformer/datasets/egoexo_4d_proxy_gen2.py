...
class Egoexo4d_proxy_gen2(torch.utils.data.Dataset):
    def __init__(self, cfg, mode, num_retries=10):
        assert mode in ["train", "val", "test"], "Split '{}' not supported for EgoExo-4D".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing EgoExo-4D proxy-gen2 %s...", mode)
        ...
        if not self.proxy_candidates:
            raise ValueError(
                "egoexo_4d_proxy_gen2 requires cfg.DATA.PROXY_CANDIDATES to contain at least one candidate."
            )
    ...
    def _construct_loader(self):
        ...
        assert len(self._path_to_videos) > 0, "Failed to load EgoExo-4D split {} from {}".format(
            split_name, path_to_file
        )
        logger.info(
            "Constructing EgoExo-4D proxy-gen2 dataloader (size: %d) from %s",
