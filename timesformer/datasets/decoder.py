...
def torchvision_decode(
    ...
    if (
        ...
        if center_frame:
            start_idx = end_idx = (fps * video_meta["video_duration"]) // 2
        else:
        ...
        video_end_pts = (
            int((end_idx + 1) * pts_per_frame)
            if center_frame
            else int(end_idx * pts_per_frame)
        )
        if video_end_pts <= video_start_pts:
            video_end_pts = video_start_pts + 1
...
def pyav_decode(
    ...
    center_buffer_size = 0
    if duration is None or frames_length is None or frames_length <= 0:
        ...
        center_idx = frames_length // 2
        video_start_pts = int(center_idx * timebase)
        video_end_pts = int((center_idx + 1) * timebase)
        if video_end_pts <= video_start_pts:
            video_end_pts = video_start_pts + 1
        center_buffer_size = 1
    else:
    ...
    if container.streams.video:
        if start is None and end is None:
            video_frames, max_pts = pyav_decode_stream(
                ...
                {"video": 0},
                buffer_size=center_buffer_size,
            )
        else:
            timebase = duration / frames_length
            start_i = start
...
def decode(
    ...
    if center_frame:
        center_idx = frames.shape[0] // 2 if decode_all_video else 0
        frames = temporal_sampling(frames, center_idx, center_idx, 1)
        return frames.repeat(num_frames, 1, 1, 1)
